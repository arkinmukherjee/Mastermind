...
// Checks the number of values in the guess match with the secret code but are in the wrong place 
{
	...
	for (int i = 0; i < guessCode.size(); i++) 
	{
		// For loop that loops through secret code values, assumes guess code is the same size as the secret code
		for (int j = 0; j < guessCode.size(); j++) 
		{
			// For loop that loops through guess code, compares all guess values against selected secret code value
...
// Output: boolean value representing if the guess is within the same range as the code generated
